class HandleDirectory::PasswordsController < HandleDirectory::BaseController
	...
	def reset_password
		...
		if @user
			if @user.role.eql?("user")# || @user.role.eql?("employee")
				User.send_password_reset(@user)
				flash[:notice] = "Your Password send to your email successfully."
				redirect_to  handle_directory_login_path
			else
		 	flash[:error] = "Invalid User."
		 	redirect_to handle_directory_passwords_forget_password_path
			end	
		else
		...
		@user = User.find_by_id(params[:id])
	 	if @user.valid_password?params[:user][:current_password] 
	 		if params[:user][:new_password].eql?(params[:user][:password_confirmation])
		 		...
		 		redirect_to handle_directory_root_path
		 	else
